...
import { createAssignment, deleteAssignment, getAllAssignments, getUpcomingAssignments, getOverdueAssignments, submitAssignment, checkSubmitted } from "../services/assignmentService";
...
export default function Assignments() {
  ...
  useEffect(() => { fetchAssignments(filterType); }, [filterType]);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setTopic(""); setDescription(""); setDueDate(""); setFile(null);
      fetchAssignments(filterType);
    } catch (err) { alert("Error creating assignment: " + (err.response?.data?.message || err.message)); }
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Delete this assignment?")) return;
    try { await deleteAssignment(id); fetchAssignments(filterType); }
    ...
    <div className="space-y-7 py-6" style={{ color: "#dae2fd" }}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage and submit your assignments</p>
        </div>
      ...
      {/* Create Form */}
      {role === "TEACHER" && (
        <form onSubmit={handleCreate} className="p-7 space-y-4"
          style={{ ...card, background: "linear-gradient(135deg, #111827 0%, #0f1e35 100%)", boxShadow: "0 8px 32px rgba(77,142,255,0.08), 0 4px 16px rgba(0,0,0,0.4)" }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center"
              style={{ background: "linear-gradient(135deg,#4d8eff,#1d4ed8)", boxShadow: "0 4px 12px rgba(77,142,255,0.3)" }}>
          ...
          <input placeholder="Topic" value={topic} onChange={(e) => setTopic(e.target.value)} required style={inputStyle}
          ...
          <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} required style={{ ...inputStyle, colorScheme: "dark" }}
            onFocus={(e) => { e.target.style.borderColor = "#4d8eff"; e.target.style.backgroundColor = "#131f35"; e.target.style.boxShadow = "0 0 0 3px rgba(77,142,255,0.1)"; }}
            onBlur={(e) => { e.target.style.borderColor = "rgba(77,142,255,0.15)"; e.target.style.backgroundColor = "#0d1424"; e.target.style.boxShadow = "none"; }} />
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 px-4 py-2.5 rounded-xl cursor-pointer text-sm font-medium transition-all hover:bg-blue-500/10"
              style={{ backgroundColor: "rgba(77,142,255,0.06)", color: "#adc6ff", border: "1px dashed rgba(77,142,255,0.25)" }}>
              <span className="material-symbols-outlined text-base">attach_file</span>
              {file ? file.name : "Attach File (optional)"}
              <input type="file" className="hidden" onChange={(e) => setFile(e.target.files[0])} />
            </label>
            {file && <button type="button" onClick={() => setFile(null)} className="text-red-400 text-xs hover:text-red-300 transition-colors">✕ Remove</button>}
          </div>
        ...
        {assignments.length === 0 ? (
          <div className="p-16 text-center" style={card}>
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
              ...
              <span className="material-symbols-outlined text-3xl text-slate-600">assignment</span>
            </div>
            <p className="text-slate-500 font-medium">No assignments found</p>
          </div>
        ) : assignments.map((a) => (
          <div key={a.id} className="p-6 transition-all duration-300 hover:-translate-y-0.5"
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-3 flex-wrap">
                  <span className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full"
                    style={{ backgroundColor: "rgba(239,68,68,0.1)", color: "#fca5a5", border: "1px solid rgba(239,68,68,0.15)" }}>
